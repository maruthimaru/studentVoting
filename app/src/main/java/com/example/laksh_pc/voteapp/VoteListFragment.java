...
public class VoteListFragment extends AppCompatActivity implements VoteListAdapter.ItemListener {
View view;
RecyclerView recyclerView;
//RecyclerView buslistRecyclerView1;
    VoteListAdapter busListAdapter;
DatabaseReference databaseReference,reference;
    private FirebaseAuth firebaseAuth;

List<UserModel> userModelArrayList =new ArrayList<>();
    List<UserModel> userModelArrayListvc =new ArrayList<>();
    private String TAG= VoteListFragment.class.getSimpleName();
...
//                startActivity(new Intent(VoteListFragment.this,UserInfoFragment.class));
    ...
    public void setAdapter(List<UserModel> busListModelList){
        busListAdapter=new VoteListAdapter(this,busListModelList,this);
...
//    }
    ...
    public void sendvote(){
        if(busListModelList!=null) {
            String n = busListModelList.getName();
            String email = firebaseAuth.getCurrentUser().getEmail().replace("@", "").replace(".", "");


            reference.child("Voted").child(email).setValue(email);
            reference.child(n).child(email).setValue(email);


            Intent startnew = new Intent(this, VoteListVCFragment.class);
